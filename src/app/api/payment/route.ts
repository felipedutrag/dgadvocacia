...
export async function POST(request: Request) {
  try {
    ...
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || "https://dgadvocacia.online";
    const webhookUrl = `${appUrl}/api/payment/webhook`;
    const description = body.description || (planName ? `${planName} - DG Advocacia` : "DG Advocacia - Assessoria & Registro INPI");
    ...
    return NextResponse.json({ 
      ...
      amount: price,
      amountCents,
      planName: planName || "Serviço DG Advocacia"
    });
