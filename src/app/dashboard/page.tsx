        ...
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-border/70 bg-background/80 px-4 sm:px-6 backdrop-blur-xl">
          ...
          {activeTab === "plans" && (
            <div className="relative space-y-10 animate-fade-in">
              ...
              {/* ── APRESENTAÇÃO INSTITUCIONAL DA ASSESSORIA ── */}
              <div className="relative rounded-3xl border border-primary/25 bg-gradient-to-br from-card/90 via-card/70 to-primary/[0.04] p-6 sm:p-10 backdrop-blur-2xl shadow-xl shadow-primary/5 space-y-4 overflow-hidden">
                {/* Textura sutil decorativa interna */}
                <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808008_1px,transparent_1px),linear-gradient(to_bottom,#80808008_1px,transparent_1px)] bg-[size:24px_24px] pointer-events-none" />

                <div className="relative z-10 flex items-center gap-2">
                  <span className="font-mono text-[10px] uppercase font-bold text-primary bg-primary/10 border border-primary/20 px-2.5 py-0.5 rounded-full flex items-center gap-1.5">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6">
                  ...
                  <div className="flex flex-col gap-2 shrink-0 w-full md:w-auto">
                    ...
                    >
                      <svg viewBox="0 0 24 24" className="size-4 fill-current shrink-0" xmlns="http://www.w3.org/2000/svg">
                  ...
                  {[
                    {
                      ...
                      title: "Radar RPI & Vigilância",
                      sub: "Monitoramento semanal ativo",
                      items: [
                        "Varredura semanal da Revista RPI",
                        "Alertas automáticos de colidência",
                        "Acompanhamento de despachos",
                        "Controle de prazos fatais",
                        "Dashboard em tempo real",
                        "Notificações críticas por e-mail",
                      ],
                    },
                    {
                      icon: Scale,
                      ...
                      sub: "Atos conduzidos por advogados",
                      items: [
                        "Cumprimento de Exigências (INPI)",
                        "Manifestação à Oposição (Art. 158)",
                        "Recurso Administrativo de 2ª Inst.",
                        "Defesa em Processos de Caducidade",
                        "Notificação Extrajudicial",
                        "Peças técnicas a terceiros",
                      ],
                    },
                    {
                      icon: FileText,
                      ...
                      sub: "Controle executivo da carteira",
                      items: [
                        "Relatório de saúde da carteira",
                        "Histórico de atos processuais",
                        "Alerta de renovação decenal",
                        "Parecer de viabilidade marcária",
                    ...
                    <div
                      key={card.title}
                      className={`rounded-2xl border bg-card/60 backdrop-blur-md overflow-hidden flex flex-col ${card.accent}`}
                    >
                      {/* Header do card */}
                      <div className={`p-5 border-b border-border/40 flex items-center gap-3`}>
                        <div className={`size-9 rounded-xl border flex items-center justify-center shrink-0 ${card.bg}`}>
                          <card.icon className={`size-4 ${card.color}`} />
                        </div>
                        <div>
                          <div className="text-sm font-bold text-foreground">{card.title}</div>
                          <div className={`text-[10px] font-mono ${card.color}`}>{card.sub}</div>
                        </div>
                      </div>
                      {/* Lista */}
                      <ul className="p-5 space-y-2.5 flex-1">
                        {card.items.map((feat) => (
                          <li key={feat} className="flex items-start gap-2.5 text-xs text-foreground/85">
                            <CheckCircle2 className={`size-3.5 shrink-0 mt-0.5 ${card.color}`} />
