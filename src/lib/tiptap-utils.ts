...
/**
 * Parser resiliente para JSON retornado por LLMs (Gemini / Claude).
 * Sanitiza quebras de linha brutas, caracteres de controle e markdown codeblocks.
 */
export function safeParseJson<T = any>(rawText: string): T {
  if (!rawText) {
    throw new Error("Texto JSON vazio.");
  }
  ...
  try {
    ...
    // 2. Sanitiza quebras de linha e caracteres de controle dentro de strings literais
    try {
      let inString = false;
      let isEscaped = false;
      let result = "";
      ...
      for (let i = 0; i < clean.length; i++) {
        ...
        if (char === '"' && !isEscaped) {
          ...
          if (char === "\n") {
            ...
            // Outros caracteres de controle ilegais em JSON
            result += "";
          } else {
            result += char;
          }
      ...
      // 3. Se ainda falhar por Unterminated string ou chaves abertas, tenta auto-completar
      try {
        let repaired = result.trim();
        // Se termina com aspas abertas ou string não terminada
        if (inString) {
          repaired += '"';
        }
        // Balancear colchetes e chaves
        const opensBracket = (repaired.match(/\[/g) || []).length;
        const closesBracket = (repaired.match(/\]/g) || []).length;
        for (let b = 0; b < opensBracket - closesBracket; b++) {
          repaired += "]";
        }

        const opensBrace = (repaired.match(/\{/g) || []).length;
        const closesBrace = (repaired.match(/\}/g) || []).length;
        for (let c = 0; c < opensBrace - closesBrace; c++) {
          repaired += "}";
        }
