...
export function parseHtmlToDocxElements(rawHtml: string, title?: string): Paragraph[] {
  ...
  while ((match = blockRegex.exec(html)) !== null) {
    ...
    if (tag === "p") {
      ...
      } else if (/nestes\s+termos/i.test(inner)) {
        // Encerramento / Nestes termos: Espaçamento generoso no topo
        paragraphs.push(
          new Paragraph({
            alignment,
      ...
      if (isCenter) {
        // NOME DA AÇÃO: 16pt (size: 32), Negrito, Centralizado com respiro simétrico
        paragraphs.push(
          new Paragraph({
      ...
      paragraphs.push(
        new Paragraph({
          ...
          spacing: { line: 280, before: 120, after: 240 },
          border: {
            left: {
              color: "0F172A",
              space: 12,
